...
import { LANGUAGES, type Localized } from "@shared/i18n";
...
describe("cobertura bilingüe de las tablas del libro", () => {
  it("encuentra pares en los seis módulos, no en uno solo", () => {
  ...
  it("el inglés no es el español repetido", () => {
    const repeated = pairs.filter(({ value }) =>
      value.es === value.en && !SAME_IN_BOTH.has(value.es) && !CITATION.test(value.es)
    );
