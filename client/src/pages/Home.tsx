...
type FinancialAssumptions = { currency?: string | null; reportingCurrency?: string | null; fxRateToReportingCurrency?: number | null; tamYearOne?: number | null; annualMarketGrowthPct?: number | null; samPct?: number | null; somPctYearOne?: number | null; somPctHorizon?: number | null; operatingMarginPct?: number | null; taxRatePct?: number | null; workingCapitalPctRevenue?: number | null; discountRatePct?: number | null; terminalGrowthPct?: number | null; modeProfiles?: Partial<Record<ModeKey, FinancialProfile>> };
...
type CountryResult = {
  ...
  financial: { status: "ok" | "insufficient_data"; currency: string | null; localCurrency: string | null; reportingCurrency: string | null; fxRateToReportingCurrency: number | null; horizonYears: number; assumptions: { taxRatePct: number | null; workingCapitalPctRevenue: number | null; discountRatePct: number | null; terminalGrowthPct: number | null }; market: { tamYearOne: number | null; tamAtHorizon: number | null; samAtHorizon: number | null; somRevenueYearOne: number | null; somRevenueAtHorizon: number | null }; alternatives: { key: ModeKey; mode: string; status: "ok" | "insufficient_data" | "not_meaningful"; roiPct: number | null; npv: number | null; paybackYear: number | null; initialInvestment: number | null; annualOperatingCost: number | null; revenueCapturePct: number | null; cumulativeFreeCashFlow: number | null; terminalValue: number | null; presentValueTerminal: number | null; annualProjection: { year: number; revenue: number; operatingProfit: number; taxes: number; changeInWorkingCapital: number; freeCashFlow: number; presentValue: number }[]; missingInputs: string[] }[]; missingInputs: string[]; methodology: string };
      ...
      <div className="studio-shell">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="mt-7">
          ...
          <TabsContent value="finance" className="tab-enter">
            <div className="finance-intro"><div><div className="eyebrow"><CircleDollarSign className="h-3.5 w-3.5" /> caso económico de entrada</div><h2>Modele el flujo de caja, no solo el tamaño del mercado.</h2><p>La evaluación incorpora impuestos, capital de trabajo, conversión de divisa y valor terminal. Todos los campos son supuestos explícitos que deben contrastarse con fiscalidad, tesorería y validación local.</p></div><div className="formula-chip">FCF = EBIT − impuestos − Δ capital de trabajo<br />TV = FCFₙ₊₁ / (r − g)</div></div>
            <div className="grid gap-6 xl:grid-cols-[.7fr_1.3fr]">
              <Card className="strategic-card"><CardHeader><div className="step-tag">MERCADO ACTIVO</div><CardTitle>{activeCandidate?.name ?? "Seleccione un mercado"}</CardTitle><CardDescription>Los supuestos financieros se guardan por país. La política de umbrales se aplica de forma consistente a toda la comparación.</CardDescription></CardHeader><CardContent>{candidates.length ? <div className="space-y-2">{candidates.map((candidate) => <button key={candidate.code} onClick={() => setActiveCountry(candidate.code)} className={`country-selector ${activeCandidate?.code === candidate.code ? "selected" : ""}`}><span className="country-code">{candidate.code}</span><span><strong>{candidate.name}</strong><small>{marketData[candidate.code]?.fdiInflowUsd ? `IED neta: ${formatBillions(marketData[candidate.code].fdiInflowUsd)}` : "Añada supuestos financieros"}</small></span><ChevronRight className="ml-auto h-4 w-4" /></button>)}</div> : <EmptyState icon={CircleDollarSign} title="Primero añada mercados" text="La estimación TAM/SAM/SOM se configura de forma separada para cada país candidato." />}</CardContent></Card>
              <div className="space-y-6">
                <Card className="financial-card"><CardHeader><div className="step-tag">{activeCandidate ? `SUPUESTOS · ${activeCandidate.code}` : "SUPUESTOS"}</div><CardTitle>Flujo de caja de entrada</CardTitle><CardDescription>El valor terminal se calcula por perpetuidad y exige que la tasa de descuento sea superior al crecimiento terminal. Deje la moneda de reporte vacía si coincide con la moneda local.</CardDescription></CardHeader><CardContent>{activeCandidate ? <div className="space-y-7"><section><h3 className="finance-section-title">Mercado y convención monetaria</h3><div className="grid gap-4 md:grid-cols-3"><Field label="Moneda local"><Input value={financialByCountry[activeCandidate.code]?.currency ?? ""} placeholder="MXN" maxLength={10} onChange={(event) => updateFinancial("currency", event.target.value)} /></Field><Field label="Moneda de reporte"><Input value={financialByCountry[activeCandidate.code]?.reportingCurrency ?? ""} placeholder="USD (opcional)" maxLength={10} onChange={(event) => updateFinancial("reportingCurrency", event.target.value)} /></Field><Field label="FX reporte / local"><Input type="number" min="0" step="0.0001" value={financialByCountry[activeCandidate.code]?.fxRateToReportingCurrency ?? ""} placeholder="Solo si difiere" onChange={(event) => updateFinancial("fxRateToReportingCurrency", event.target.value)} /></Field><Field label="TAM año 1"><Input type="number" min="0" value={financialByCountry[activeCandidate.code]?.tamYearOne ?? ""} placeholder="Ej. 50000000" onChange={(event) => updateFinancial("tamYearOne", event.target.value)} /></Field><Field label="Crecimiento anual (%)"><Input type="number" value={financialByCountry[activeCandidate.code]?.annualMarketGrowthPct ?? ""} placeholder="Ej. 6" onChange={(event) => updateFinancial("annualMarketGrowthPct", event.target.value)} /></Field><Field label="SAM (% TAM)"><Input type="number" min="0" max="100" value={financialByCountry[activeCandidate.code]?.samPct ?? ""} placeholder="Ej. 30" onChange={(event) => updateFinancial("samPct", event.target.value)} /></Field></div></section>
                  <section><h3 className="finance-section-title">Conversión de mercado a flujo libre</h3><div className="mt-4 grid gap-4 md:grid-cols-3"><Field label="SOM año 1 (% SAM)"><Input type="number" min="0" max="100" value={financialByCountry[activeCandidate.code]?.somPctYearOne ?? ""} placeholder="Ej. 1" onChange={(event) => updateFinancial("somPctYearOne", event.target.value)} /></Field><Field label={`SOM año ${horizonYears} (% SAM)`}><Input type="number" min="0" max="100" value={financialByCountry[activeCandidate.code]?.somPctHorizon ?? ""} placeholder="Ej. 4" onChange={(event) => updateFinancial("somPctHorizon", event.target.value)} /></Field><Field label="Margen operativo (%)"><Input type="number" value={financialByCountry[activeCandidate.code]?.operatingMarginPct ?? ""} placeholder="Ej. 18" onChange={(event) => updateFinancial("operatingMarginPct", event.target.value)} /></Field><Field label="Tasa fiscal (%)"><Input type="number" min="0" max="100" value={financialByCountry[activeCandidate.code]?.taxRatePct ?? ""} placeholder="Ej. 25" onChange={(event) => updateFinancial("taxRatePct", event.target.value)} /></Field><Field label="Capital de trabajo (% ingresos)"><Input type="number" value={financialByCountry[activeCandidate.code]?.workingCapitalPctRevenue ?? ""} placeholder="Ej. 12" onChange={(event) => updateFinancial("workingCapitalPctRevenue", event.target.value)} /></Field><Field label="Tasa de descuento (%)"><Input type="number" min="0" value={financialByCountry[activeCandidate.code]?.discountRatePct ?? ""} placeholder="Ej. 10" onChange={(event) => updateFinancial("discountRatePct", event.target.value)} /></Field><Field label="Crecimiento terminal (%)"><Input type="number" value={financialByCountry[activeCandidate.code]?.terminalGrowthPct ?? ""} placeholder="Ej. 2" onChange={(event) => updateFinancial("terminalGrowthPct", event.target.value)} /></Field></div></section>
                  ...
                  <div className="finance-note"><CircleAlert className="h-4 w-4" /><p><strong>Convención:</strong> ROI = (flujo libre acumulado − inversión inicial) / inversión inicial y excluye el valor terminal. NPV incluye los flujos libres descontados y el valor terminal. No se incorporan financiación, depreciación, amortización ni cambios fiscales futuros; añádalos en un modelo corporativo si son materiales.</p></div></div> : <EmptyState icon={CircleDollarSign} title="Sin país activo" text="Seleccione un mercado en la columna izquierda para introducir el caso económico." />}</CardContent></Card>
