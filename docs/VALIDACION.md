...
## Resiliencia al añadir varios países

Se identificó que cada país lanzaba simultáneamente diez consultas macroeconómicas y dos consultas financieras; al añadir candidatos consecutivos, una respuesta de indisponibilidad transitoria podía llegar como texto plano y el cliente la mostraba como un error de JSON. La corrección limita World Bank a tres peticiones concurrentes, reintenta códigos HTTP transitorios y respuestas no JSON, y serializa las actualizaciones automáticas de cada país en la interfaz. La tabla marca cada fila como “Actualizando” durante la cola y se realiza un reintento adicional a nivel de interfaz cuando la respuesta temporal proviene de la ruta de la aplicación. La verificación en vivo de Alemania, Francia y México completó datos macro, impuestos y FX de forma consecutiva en 4,7, 2,8 y 3,4 segundos, respectivamente. La suite aumentó a 16 pruebas aprobadas; TypeScript y la compilación de producción finalizaron correctamente.
